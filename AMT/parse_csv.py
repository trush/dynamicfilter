...
find_pairs_csv = csv.writer(open('FIND_PAIRS_RESULTS.csv', 'w'), delimiter=',')
find_pairs_csv.writerow(["HIT ID","HOTEL","ASSIGNMENT ID","TIME TAKEN","WORKER ANSWER"])
for assignment in hit_csv:
    if assignment[4] == "( list_secondary)":
        find_pairs_csv.writerow([assignment[0],assignment[2],assignment[5],assignment[7],assignment[8]])
